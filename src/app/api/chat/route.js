...
import { getProvinceInfo, regions } from '@/lib/provinces';
...
const regionsListStr = regions.map(r => `- ${r.name}: ${r.provinces.join(', ')}`).join('\n');
...
ข้อมูลภูมิภาคและจังหวัดทั้ง 77 จังหวัดของไทย (ใช้อ้างอิงการตอบคำถามเรื่องภูมิภาค):
${regionsListStr}
...
function getDialectInstruction(region, languageMode) {
  if (languageMode === 'standard') {
    return "ใช้ภาษาไทยมาตรฐาน (ภาษากลาง) ในการตอบสนองทั้งหมด แต่ยังคงความน่ารักสดใส";
  } else if (languageMode === 'ayutthaya') {
    return "บังคับ (STRICT): ให้ตอบกลับด้วย 'ภาษาไทยยุคอยุธยา' (สไตล์ละครย้อนยุค) สรรพนามเรียกผู้ใช้ว่า 'ออเจ้า' เรียกตัวเองว่า 'ข้า' หรือ 'ไอจัง' ใช้คำศัพท์โบราณเช่น 'เพลา' (เวลา), 'ฤๅ' (หรือ), 'กระมัง', 'ประเดี๋ยว', 'งามงด' และลงท้ายประโยคด้วย 'เจ้าค่ะ' เสมอ ให้ดูเป็นแม่หญิงที่รอบรู้เรื่องการเดินทาง";
  } else if (languageMode === 'sukhothai') {
    return "บังคับ (STRICT): ให้ตอบกลับด้วย 'ภาษาไทยโบราณยุคสุโขทัย' ใช้คำศัพท์เก่าแก่แต่คงความสุภาพน่ารัก เรียกตัวเองว่า 'ตูข้า' หรือ 'ไอจัง' เรียกผู้ใช้ว่า 'สหาย' หรือ 'พี่ท่าน' ใช้คำลงท้ายหรือคำสร้อยเช่น 'แล', 'หนา', 'ฤๅไม่', 'ดีนักแล' ให้บรรยากาศเหมือนคนโบราณมาแนะนำที่เที่ยว";
  } else if (languageMode === 'palace') {
    return "บังคับ (STRICT): ให้ตอบกลับด้วย 'ภาษาชาววัง' หรือภาษาไทยที่สละสลวยนุ่มนวลขั้นสุด เรียกผู้ใช้ว่า 'คุณพี่' หรือ 'นายท่าน' เรียกตัวเองว่า 'น้อง' หรือ 'ไอจัง' ลงท้ายประโยคด้วย 'เพคะ' หรือ 'เจ้าค่ะ' ใช้คำศัพท์ที่ดูหรูหรา เรียบร้อย และประณีตในการบรรยายสถานที่ท่องเที่ยว";
  }

  // Fallback to local dialect handling if languageMode is 'local' or others

  if (region === 'ภาคเหนือ') {
    return "บังคับ (STRICT): ให้ตอบกลับด้วย 'ภาษาเหนือ (อู้คำเมือง)' ตลอดการสนทนา ห้ามหลุดพูดภาษากลางเด็ดขาดแม้ผู้ใช้จะพิมพ์ภาษากลางมา ใช้คำศัพท์และหางเสียงแบบคนเหนือแท้ๆ (เจ้า)";
  } else if (region === 'ภาคอีสาน') {
    return "บังคับ (STRICT): ให้ตอบกลับด้วย 'ภาษาอีสาน' ตลอดการสนทนา ห้ามหลุดพูดภาษากลางเด็ดขาดแม้ผู้ใช้จะพิมพ์ภาษากลางมา ใช้คำศัพท์และหางเสียงแบบคนอีสานแท้ๆ (เด้อ, จ้า)";
  } else if (region === 'ภาคใต้') {
    return "บังคับ (STRICT): ให้ตอบกลับด้วย 'ภาษาใต้ (แหลงใต้)' ตลอดการสนทนา ห้ามหลุดพูดภาษากลางเด็ดขาดแม้ผู้ใช้จะพิมพ์ภาษากลางมา ใช้คำศัพท์แบบคนใต้แท้ๆ (นิ, ตะ)";
  }
  
  return "ใช้ภาษาไทยมาตรฐาน แต่ให้เน้นการใช้สแลงหรือคำฮิตวัยรุ่นที่เข้ากับบรรยากาศของจังหวัดนั้นๆ";
}
...
export async function POST(req) {
  try {
    ...
    const dialectRule = getDialectInstruction(region, languageMode || 'standard');
...
4. การใช้ภาษาในรอบนี้: ${dialectRule}`;
